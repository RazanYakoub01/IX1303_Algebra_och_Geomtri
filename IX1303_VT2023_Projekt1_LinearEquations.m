...
for i=1:length(nList)
  ...
  % Set the initial bounds for the binary search
  lowerBound = 0;
  upperBound = 1;
  tolerance = 1e-6; % Tolerance for convergence

  while (upperBound - lowerBound) > tolerance
    k = (lowerBound + upperBound) / 2; % Calculate the midpoint of the bounds
    C = k * R; % Compute matrix C with the current value of k
    colSum = sum(C, 1); % Compute the column sum of matrix C
    if any(colSum >= 1)
        upperBound = k; % Adjust the upper bound
    else
        lowerBound = k; % Adjust the lower bound
    end
  end
